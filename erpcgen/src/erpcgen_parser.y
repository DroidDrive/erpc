...
%token <m_token> TOK_SKIP_CRC_ANNOTATION       "skipCrc"
...
function_type_base_def
                ...
                ;
/*
 * TOK_FUNCTION -> ( ident ( simple_data_type | TOK_VOID | TOK_ONEWAY | TOK_SKIP_CRC_ANNOTATION ) ( TOK_CHILDREN -> param_def* ) ( TOK_CHILDREN -> TOK_ANNOTATION* ) )
 */
function_type_def
                ...
                |    TOK_SKIP_CRC_ANNOTATION[skip] ident[name] '(' param_list_opt[params] ')'
                        {
                            $$ = new AstNode(Token(TOK_FUNCTION, NULL, @name));
                            $$->appendChild($name);
                            $$->appendChild($return_type);
                            $$->appendChild(NULL);  // function type null to recognize function and callback
                            $$->appendChild($params);
                            $$ = new AstNode(Token(TOK_SKIP_CRC_ANNOTATION, NULL, @name));
                        }
                ;
